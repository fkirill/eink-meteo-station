...
func loadImages() error {
	...
	digitsImageSize = make([]image.Point, 60, 60)
	for i := 0; i < 60; i++ {
		...
		digitsImageSize[i] = img.Bounds().Max
	...
	clockWidgetSize = image.Point{X: digitsImageSize[0].X*3 + colonSize.X*2, Y: colonSize.Y}
	...
	utils.DrawImage(initialRaster, clockWidgetSize, image.Point{X: digitsImageSize[0].X}, colonImage, colonSize)
	utils.DrawImage(initialRaster, clockWidgetSize, image.Point{X: digitsImageSize[0].X*2 + colonSize.X}, colonImage, colonSize)
...
func (c *clockRenderable) Render() error {
	...
	if c.minute != nextMinute {
		c.drawNumber(digitsImageSize[0].X+colonSize.X, nextMinute)
	}
	if c.second != nextSecond {
		c.drawNumber(digitsImageSize[0].X*2+colonSize.X*2, nextSecond)
	}
	c.hour = nextHour
	c.minute = nextMinute
	c.second = nextSecond
...
func (c *clockRenderable) drawNumber(xOffset int, number int) {
	utils.DrawImage(c.raster, c.Size(), image.Point{X: xOffset, Y: 0}, digitImages[number], digitsImageSize[number])
}
